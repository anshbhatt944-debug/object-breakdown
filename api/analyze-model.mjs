...
/*
 * Vercel serverless entry point for POST /api/analyze-model.
 *
 * The Groq API key is read only from Vercel's server-side
 * environment variable: GROQ_API_KEY.
 */

export default async function handler(req, res) {
  // CORS is harmless for same-origin usage and helps local/testing.
  ...
  try {
    ...
    // Depending on the Vercel runtime, req.body may be an object,
    // a JSON string, or absent.
...
// Exporting the current model name is not required by the frontend,
// but it makes the function easy to sanity-check when debugging.
export { GROQ_MODEL };
