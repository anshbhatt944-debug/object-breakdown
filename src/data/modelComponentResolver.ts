...
function prettyMeshName(componentId: string, objectId: string): { name: string; category: string } {
  ...
  if (objectId === 'wristwatch') {
    ...
    if (/cylinder.*plastc pink/.test(text)) return { name: 'Polymer Spacer / Retainer', category: 'Structural' };
    if (/cylinder.*inside metal/.test(text)) return { name: 'Internal Metal Spacer / Collar', category: 'Structural' };
