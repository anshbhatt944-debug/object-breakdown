    ...
    <section className="landing-hero pt-28 sm:pt-32 px-4 sm:px-6 pb-12 max-w-[1500px] mx-auto">
      <div className="landing-hero-grid">
        <div className="hero-copy">
          ...
          <div className="hero-tags"><span>Try</span>{['Smartphone', 'Mechanical Watch', 'Ballpoint Pen', 'Keyboard'].map((item) => <button key={item} onClick={() => onSearch(item)}>{item}</button>)}</div><label className="inline-flex mt-5 cursor-pointer items-center gap-2 rounded-xl border border-[#00f2ad]/40 bg-[#00f2ad]/10 px-4 py-3 text-sm font-bold text-[#00f2ad] hover:bg-[#00f2ad]/20 transition-colors"><Upload className="w-4 h-4" /> Upload your own .GLB<input className="hidden" type="file" accept=".glb,.gltf" onChange={(e) => { const file = e.target.files?.[0]; if (file) onUploadModel(file); e.currentTarget.value = ''; }} /></label>
